...
public class CitaMedicaDAOImpl implements CitaMedicaDAO {
    ...
    public CitaMedica load(Integer id) {
        ...
        try (Connection con = DBManager.getInstance().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    Paciente paciente = new Paciente(null, null, null, null, null, null, null, rs.getInt(5), null,
                            false, false);
                    Medico medico = new Medico(null, null, null, null, null, null, null, rs.getInt(6), null, null, null,
    ...
    public CitaMedica save(CitaMedica t) {
        ...
        try (Connection con = DBManager.getInstance().getConnection();
            ...
            ps.setObject(4, t.getPaciente() != null ? t.getPaciente().getId() : null);
            ps.setObject(5, t.getDoctor() != null ? t.getDoctor().getId() : null);
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next())
                    t.getIdCita(rs.getInt(1)); // El modelo dice getIdCita pero le enviamos id como setter
            }
            return t;
        } catch (SQLException e) {
            e.printStackTrace();
